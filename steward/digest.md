...
- **Supervisor is NOT running on his PC** (2nd confirmed incident): the 15-min GitHub-raw re-pull never fired; both worker-1.9 deploys needed sealed force-restarts (download → kill python matching clippy-worker → `Start-Process` explicit relaunch — env inherited via the PowerShell child, do it this way, never rely on autostart). OPEN: get the supervisor scheduled task actually registered/running.
