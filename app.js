...
<div style="text-align:center;font-size:7px;color:var(--faint);margin-top:8px">NEXUS — Generated ${new Date().toLocaleDateString()} — Use dry-erase marker, scan to log</div>
  ...
  setupNav() {
    ...
    // ── Back-navigation state (browser back button + swipe-back gesture) ──
    // _navStack is our own source of truth for "where back goes" — immune to
    // the replaceState URL-cleanups equipment/inventory do. Each forward nav
    // also pushes one browser history entry so the back button / swipe can
    // drive popstate. The two stay in lockstep (nothing else calls pushState).
    let _navStack = [];
    let _navCurrentView = 'home';
    ...
    const switchTo = (view) => {
      ...
      // ── Back-navigation bookkeeping ──
      // On genuine forward navigation, remember where we were and add a
      // browser history entry so the back button / swipe-back can pop one
      // screen. popstate-driven navigation sets _navSuppressPush so we don't
      // re-push the view we're popping back to.
      if (!_navSuppressPush && _shown !== _navCurrentView) {
        _navStack.push(_navCurrentView);
        try { history.pushState({ nxBack: true }, ''); } catch (_) {}
      }
      _navCurrentView = _shown;
      if (NX._syncBackBtn) NX._syncBackBtn();
    ...
    // ── BACK NAVIGATION ─────────────────────────────────────────────────
    // Both the browser/hardware back button and the swipe-back gesture route
    // through popstate. goBack() is exposed for any UI that wants a back
    // affordance. "Back" walks _navStack one screen at a time until home;
    // at home there's nothing left to pop and a further back exits the app.
    NX.goBack = () => { if (_navStack.length > 0) history.back(); };

    window.addEventListener('popstate', () => {
      // Let hash-based overlays (e.g. the equipment audit at #audit) own
      // their own back; only handle plain view history here.
      if (location.hash) return;
      if (_navStack.length === 0) return;     // at root (home) — let the app exit
      const prev = _navStack.pop();
      _navSuppressPush = true;
      try { switchTo(prev); } finally { _navSuppressPush = false; }
    });

    // Swipe-back gesture (touch). A clear leftward swipe pops one screen.
    // Guards keep it from ever fighting real interaction: ignored on inputs,
    // open sheets/dials, canvases, and any horizontally-scrollable element
    // (board lanes, the nav rail, carousels). Routes through history.back()
    // so swipe and the browser back button share one code path.
    (function wireSwipeBack() {
      const BACK_DIR = -1;          // -1 = swipe LEFT goes back. Flip to +1 for an iOS-style rightward swipe.
      const DIST = 70;              // min horizontal travel (px)
      const RATIO = 1.5;            // horizontal must dominate vertical by this factor
      const MAX_TIME = 700;         // a swipe, not a slow drag (ms)
      ...
      const hScrollable = (el) => {
        for (let n = el; n && n !== document.body && n.nodeType === 1; n = n.parentElement) {
          let s; try { s = getComputedStyle(n); } catch (_) { return false; }
      ...
      document.addEventListener('touchstart', (e) => {
        ...
        const t = e.touches[0];
        const tgt = e.target;
        blocked = !!(tgt && tgt.closest && tgt.closest(
          'input,textarea,select,[contenteditable="true"],canvas,' +
          '.eq-bulk-sheet,.dispatch-sheet,.nxvf-sheet,.nx-speed-dial,.nxrm-vendor-form-overlay,' +
          'input[type="range"],.eq-sched-phases'
        )) || hScrollable(tgt);
      ...
      document.addEventListener('touchend', (e) => {
        ...
        if (Math.abs(dx) < DIST) return;
        if (Math.abs(dx) < Math.abs(dy) * RATIO) return;   // not horizontal enough
        if ((dx < 0 ? -1 : 1) !== BACK_DIR) return;        // wrong direction
        NX.goBack();
      }, { passive: true });
    })();

    // Visible back affordance in the masthead. Shows only when there's a
    // screen to go back to (so it's absent at home). Works on every size,
    // and gives desktop a one-tap back next to the brand. Injected here so
    // it lives wherever the masthead is.
    (function wireBackButton() {
      const nav = document.querySelector('#appWrap > .nav') || document.querySelector('.nav');
      if (!nav) return;
      let btn = document.getElementById('navBack');
      if (!btn) {
        ...
        btn.title = 'Back';
        btn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg>';
        btn.style.cssText = 'display:none;align-items:center;justify-content:center;width:38px;height:38px;flex:0 0 auto;margin-right:4px;border:none;background:none;color:var(--text);border-radius:9px;cursor:pointer;-webkit-tap-highlight-color:transparent';
        btn.addEventListener('click', () => NX.goBack());
        btn.addEventListener('mouseenter', () => { btn.style.background = 'var(--nx-gold-faint)'; });
