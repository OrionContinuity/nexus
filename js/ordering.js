...
Thanks for your help sorting this out.`;
  ...
  function renderItemRow(item) {
    ...
    return `
      <div class="ved-item-row${hasPar ? ' has-par' : ''}" data-item-id="${esc(item.id)}">
        ...
        <!-- v362 — centered chevron: opens the full edit form for this item (a quick, obvious
             "open details/edit" affordance, same destination as tapping the name). -->
        <button type="button" class="ved-item-expand" data-edit-item="${esc(item.id)}" aria-label="Edit item details">
  ...
  function wireItemListHandlers() {
    ...
    // v362 — chevron: open the full edit form for this item (same as tapping the name).
    list.querySelectorAll('.ved-item-expand').forEach(btn => {
      btn.addEventListener('click', () => {
        catalogState.editingItemId = btn.dataset.editItem;
        renderItemsAreaOnly();
        setTimeout(() => {
          const form = list.querySelector('.ord-vitem-editing');
          if (form) form.scrollIntoView({ behavior: 'smooth', block: 'center' });
