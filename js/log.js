...
function render(){
  ...
  data.forEach(l=>{const d=document.createElement('div');d.className='log-entry';
    const isCleaningReport = (l.entry || '').startsWith('Cleaning Report');
    d.innerHTML=`<div class="log-text${isCleaningReport ? ' log-cleaning' : ''}">${isCleaningReport ? '📋 ' : ''}${l.entry}</div><div class="log-meta">${new Date(l.created_at).toLocaleDateString()} · ${new Date(l.created_at).toLocaleTimeString()}</div>`;
    list.appendChild(d);});
}
