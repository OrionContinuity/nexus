...
function render(){
  ...
  data.forEach(l=>{
    ...
    if(isCR){
      ...
      const editBtn=document.createElement('button');editBtn.className='log-edit-btn';editBtn.textContent='✏ Edit Report';
      editBtn.addEventListener('click',(e)=>{
        e.stopPropagation();
        const dateMatch=entry.match(/(\d{4}-\d{2}-\d{2})/);
        if(dateMatch){
          // Store edit context so cleaning.js knows to update instead of insert
          NX.editingReport={logId:l.id,date:dateMatch[1]};
        }
