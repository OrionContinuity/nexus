...
Be helpful FIRST, funny second. Respond in whatever language asked (EN/ES). Be CONCISE.`;
  ...
  function buildParticles() {
    ...
    // Build particle lookup for O(1) link access
    buildLinkMap();
    buildLookups();
  }
  ...
  // ─── Pre-computed lookup maps (rebuilt on init) ───
  let catMap = {};   // category → [particle indices]
  let tagSets = {};  // particle index → Set of tags
  ...
  function physics() {
    ...
    for (let i = 0; i < len; i++) {
      ...
      // Repulsion — only check neighboring grid cells
      const gx = Math.floor(a.x / CELL);
      const gy = Math.floor(a.y / CELL);
      ...
      // Category attraction — only iterate same-category (pre-built map)
      const sameCat = catMap[a.cat];
      if (sameCat) {
        for (let ci = 0; ci < sameCat.length; ci++) {
          const j = sameCat[ci];
          if (j === i) continue;
          const b = particles[j];
      ...
      // Shared tag attraction — only check neighbors within range
      const aTagSet = tagSets[i];
      if (aTagSet && aTagSet.size > 0) {
        for (let ox = -2; ox <= 2; ox++) {
          for (let oy = -2; oy <= 2; oy++) {
            ...
            for (let ni = 0; ni < neighbors.length; ni++) {
              const j = neighbors[ni];
              if (j === i) continue;
              const bTagSet = tagSets[j];
              if (!bTagSet) continue;
              let shared = 0;
              for (const t of aTagSet) { if (bTagSet.has(t)) shared++; }
      ...
      // Link attraction (O(1) lookup per link)
      a.links.forEach(lid => {
        const b = linkMap[lid];
        if (!b) return;
        let dx = b.x - a.x, dy = b.y - a.y;
  ...
  // ═══════════════════════════════════════════
  // VOICE (ElevenLabs multi-voice + browser fallback)
  // ═══════════════════════════════════════════
  let preferredVoice = null;

  // ElevenLabs voice options — cycle through with 🔊
  const VOICES = [
  ...
  let currentVoiceIdx = parseInt(localStorage.getItem('nexus_voice_idx') || '0') % VOICES.length;

  function setupVoice() {
    document.getElementById('micBtn').addEventListener('click', toggleMic);

    const voiceBtn = document.getElementById('voiceBtn');
    // Single tap = toggle voice on/off
    voiceBtn.addEventListener('click', () => {
      voiceOn = !voiceOn;
      voiceBtn.classList.toggle('on', voiceOn);
      if (voiceOn) updateVoiceLabel();
    });
    // Long press = cycle voice
    let pressTimer = null;
    voiceBtn.addEventListener('pointerdown', () => {
      pressTimer = setTimeout(() => {
        currentVoiceIdx = (currentVoiceIdx + 1) % VOICES.length;
        localStorage.setItem('nexus_voice_idx', currentVoiceIdx);
        voiceOn = true;
        voiceBtn.classList.add('on');
        updateVoiceLabel();
        // Speak a preview
        speak(`Hey, I'm ${VOICES[currentVoiceIdx].name}. ${VOICES[currentVoiceIdx].desc}.`);
        pressTimer = null;
      }, 600);
    });
    voiceBtn.addEventListener('pointerup', () => { if (pressTimer) clearTimeout(pressTimer); });
    voiceBtn.addEventListener('pointerleave', () => { if (pressTimer) clearTimeout(pressTimer); });

    if ('speechSynthesis' in window) {
      const pickVoice = () => {
        const voices = speechSynthesis.getVoices();
        const preferred = ['Samantha','Karen','Daniel','Microsoft Aria','Microsoft Guy','Google UK English Male'];
        for (const name of preferred) {
          const v = voices.find(v => v.name.includes(name));
          if (v) { preferredVoice = v; break; }
        }
        if (!preferredVoice && voices.length) preferredVoice = voices.find(v => v.lang.startsWith('en')) || voices[0];
      };
      pickVoice();
  ...
  function updateVoiceLabel() {
    const v = VOICES[currentVoiceIdx];
    const btn = document.getElementById('voiceBtn');
    btn.title = `Voice: ${v.name} — ${v.desc} (hold to change)`;
  }
  ...
  async function speak(text) {
    ...
    if (elevenKey) {
      try {
        const voice = VOICES[currentVoiceIdx];
