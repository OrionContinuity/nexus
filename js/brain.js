/* NEXUS Brain v5.2 — HUD layout, 10 voices */
(function(){
  ...
  const PERSONA=`You are NEXUS, the AI ops brain for Alfredo Ortiz — Suerte, Este, Bar Toti (Austin TX). You have email source references for nodes. Cite sources when asked "why" or "where from". Sharp, concise, warm. Dry wit. Helpful FIRST. EN/ES.`;

  function init(){resize();buildParticles();for(let i=0;i<300;i++)physics();setupChat();setupSearch();setupCanvas();setupVoice();setupListView();setupContractorEvents();checkApiKey();draw();}
  ...
  function physics(){
    const len=particles.length,cx=W/2,cy=H/2,ih=hoverNode!==null,CELL=200,grid={};
    for(let i=0;i<len;i++){const a=particles[i],k=Math.floor(a.x/CELL)+','+Math.floor(a.y/CELL);if(!grid[k])grid[k]=[];grid[k].push(i);}
    for(let i=0;i<len;i++){const a=particles[i];if(ih&&Math.hypot(a.x-hoverNode.x,a.y-hoverNode.y)<100){a.vx*=0.05;a.vy*=0.05;continue;}
    ...
    const sc=catMap[a.cat];if(sc)for(let ci=0;ci<sc.length;ci++){const j=sc[ci];if(j===i)continue;const b=particles[j];let dx=b.x-a.x,dy=b.y-a.y,d=Math.sqrt(dx*dx+dy*dy)||1;a.vx+=dx/d*(d-IDC)*ATT;a.vy+=dy/d*(d-IDC)*ATT;}
    const aT=tagSets[i];if(aT&&aT.size>0)for(let ox=-2;ox<=2;ox++)for(let oy=-2;oy<=2;oy++){const nb=grid[(gx+ox)+','+(gy+oy)];if(!nb)continue;for(let ni=0;ni<nb.length;ni++){const j=nb[ni];if(j===i)continue;const bT=tagSets[j];if(!bT)continue;let sh=0;for(const t of aT)if(bT.has(t))sh++;if(!sh)continue;const b=particles[j];let dx=b.x-a.x,dy=b.y-a.y,d=Math.sqrt(dx*dx+dy*dy)||1;a.vx+=dx/d*(d-IDT)*ATT*sh*1.2;a.vy+=dy/d*(d-IDT)*ATT*sh*1.2;}}
    a.links.forEach(lid=>{const b=linkMap[lid];if(!b)return;let dx=b.x-a.x,dy=b.y-a.y,d=Math.sqrt(dx*dx+dy*dy)||1;a.vx+=dx/d*(d-IDL)*LINK;a.vy+=dy/d*(d-IDL)*LINK;});
    const nA=Math.min(a.access/60,1);a.vx+=(cx-a.x)*CPULL*(0.3+nA*0.7);a.vy+=(cy-a.y)*CPULL*(0.3+nA*0.7);
    a.vx*=DAMP;a.vy*=DAMP;const sp=Math.sqrt(a.vx*a.vx+a.vy*a.vy);if(sp>MAXV){a.vx=a.vx/sp*MAXV;a.vy=a.vy/sp*MAXV;}
    a.x+=a.vx;a.y+=a.vy;if(a.x<80)a.vx+=1;if(a.x>W-80)a.vx-=1;if(a.y<80)a.vy+=1;if(a.y>H-80)a.vy-=1;}
  }

  function draw(){
    time+=0.008;physics();ctx.save();ctx.fillStyle='#121214';ctx.fillRect(0,0,W,H);ctx.translate(transform.x,transform.y);ctx.scale(transform.scale,transform.scale);
    const cx=W/2,cy=H/2,isA=activatedNodes.size>0;
    particles.forEach(a=>a.links.forEach(lid=>{const b=linkMap[lid];if(!b)return;const hot=(activatedNodes.has(a.id)&&activatedNodes.has(b.id))||(searchHits.has(a.id)&&searchHits.has(b.id)),dim=isA&&!hot;const mx=(a.x+b.x)/2+Math.sin(time*.5+a.id)*5,my=(a.y+b.y)/2+Math.cos(time*.4+b.id)*5;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.quadraticCurveTo(mx,my,b.x,b.y);ctx.strokeStyle=hot?cc(a.cat,.55):dim?'rgba(212,182,138,.02)':'rgba(212,182,138,.12)';ctx.lineWidth=hot?2.5:1.2;ctx.stroke();if(hot)for(let k=0;k<2;k++){const pt=((time*.5+a.id*.1+k*.5)%1),t2=pt,t1=1-t2;ctx.beginPath();ctx.arc(t1*t1*a.x+2*t1*t2*mx+t2*t2*b.x,t1*t1*a.y+2*t1*t2*my+t2*t2*b.y,2.5,0,Math.PI*2);ctx.fillStyle=cc(a.cat,.8-pt*.5);ctx.fill();}}));
    particles.forEach(a=>{const hit=searchHits.has(a.id)||activatedNodes.has(a.id),act=activeNode&&activeNode.id===a.id,dim=isA&&!hit&&!act;ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(a.x,a.y);ctx.strokeStyle=`rgba(212,182,138,${hit?.2:act?.25:dim?.008:.03})`;ctx.lineWidth=hit||act?1.2:.4;ctx.stroke();});
    const br=Math.sin(time*1.1),pr=30+br*4;ctx.shadowBlur=30;ctx.shadowColor='rgba(212,182,138,.4)';ctx.beginPath();ctx.arc(cx,cy,pr,0,Math.PI*2);ctx.fillStyle='#18181c';ctx.fill();ctx.strokeStyle=`rgba(212,182,138,${.5+br*.2})`;ctx.lineWidth=2;ctx.stroke();ctx.shadowBlur=0;ctx.fillStyle=`rgba(212,182,138,${.7+br*.1})`;ctx.font='600 14px JetBrains Mono';ctx.textAlign='center';ctx.fillText('NEXUS',cx,cy+5);
    particles.forEach(a=>{const hit=searchHits.has(a.id)||activatedNodes.has(a.id),act=activeNode&&activeNode.id===a.id,dim=isA&&!hit&&!act,p=Math.sin(time*1.5+a.id*.6);const sx=a.x*transform.scale+transform.x,sy=a.y*transform.scale+transform.y;if(sx<-80||sx>W+80||sy<-80||sy>H+80)return;const nr=act?AR+p*2:hit?SR+p*1.5:dim?5:BR+p*.5;ctx.shadowBlur=hit||act?18:6;ctx.shadowColor=hit||act?cc(a.cat,.8):cc(a.cat,.15);ctx.beginPath();ctx.arc(a.x,a.y,nr,0,Math.PI*2);ctx.fillStyle=cc(a.cat,(act?.9:hit?.7:dim?.06:.35)+p*.02);ctx.fill();ctx.strokeStyle=cc(a.cat,act?.6:hit?.45:dim?.03:.12);ctx.lineWidth=act?1.5:.8;ctx.stroke();ctx.shadowBlur=0;const sr=nr*transform.scale;if(sr<5&&!hit&&!act)return;const label=a.node.name.length>22?a.node.name.slice(0,20)+'…':a.node.name,la=act?.95:hit?.85:dim?.05:.45;ctx.font=`${act?'500 ':'300 '}11px "Libre Franklin"`;ctx.textAlign='center';const tw=ctx.measureText(label).width,tx=a.x,ty=a.y-nr-7;if(la>.08){ctx.fillStyle=`rgba(0,0,0,${la*.55})`;ctx.beginPath();ctx.roundRect(tx-tw/2-6,ty-8,tw+12,16,4);ctx.fill();}ctx.fillStyle=`rgba(236,233,225,${la})`;ctx.fillText(label,tx,ty+3,220);});
  ...
  function openPanel(n){activeNode=n;document.getElementById('npCat').textContent=n.category.toUpperCase();document.getElementById('npName').textContent=n.name;document.getElementById('npTags').textContent=(n.tags||[]).map(t=>'#'+t).join('  ');document.getElementById('npNotes').textContent=n.notes||'';
  ...
  function setupListView(){document.getElementById('listToggle').addEventListener('click',()=>{listViewOpen=!listViewOpen;const lv=document.getElementById('listView'),btn=document.getElementById('listToggle');if(listViewOpen){btn.classList.add('on');lv.classList.add('open');buildListFilters();renderList();}else{btn.classList.remove('on');lv.classList.remove('open');}});document.querySelectorAll('.sort-btn').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('.sort-btn').forEach(x=>x.classList.remove('active'));b.classList.add('active');listSortMode=b.dataset.sort;renderList();}));}
  ...
  // Contractor events (same as before, compacted)
  ...
  // ═══ CHAT (HUD version) ═══
  function setupChat(){
    const i=document.getElementById('chatInput'),s=document.getElementById('chatSend'),hud=document.getElementById('chatHud'),dim=document.getElementById('brainDim'),r=document.getElementById('resetBtn');
    i.addEventListener('input',()=>{s.disabled=!i.value.trim();});
    i.addEventListener('focus',()=>{dim.classList.add('active');hud.classList.add('expanded');});
    i.addEventListener('blur',()=>{if(!i.value.trim()&&!chatActive){dim.classList.remove('active');hud.classList.remove('expanded');}});
  ...
  async function handleTask(task) {
    if (task.type === 'log') {
      const { error } = await NX.sb.from('daily_logs').insert({ entry: task.content });
      return error ? 'Failed to log.' : `Logged: "${task.content}"`;
    }
    if (task.type === 'card') {
      const { error } = await NX.sb.from('kanban_cards').insert({ title: task.content, column_name: 'todo' });
      return error ? 'Failed to create card.' : `Card created: "${task.content}"`;
    }
    if (task.type === 'event') {
      // Let AI parse the scheduling details
  ...
  async function getCtx(q) {
    ...
    const sc = NX.nodes.map(n => {
      let s = 0;
      const t = (n.name + ' ' + n.category + ' ' + (n.tags || []).join(' ') + ' ' + (n.notes || '') + ' ' + JSON.stringify(n.sources || n.source_emails || [])).toLowerCase();
      w.forEach(x => { if (t.includes(x)) s += t.split(x).length - 1; });
      if (n.name.toLowerCase().includes(q.toLowerCase())) s += 10;
    ...
    // Fire synapses + track access
    const relIds = rel.map(n => n.id);
    activatedNodes = new Set(relIds);
  ...
  async function askAI() {
    ...
    // Check for voice-to-task commands
    ...
    if (task) {
      ...
      if (result) {
        ...
        try { await NX.sb.from('chat_history').insert({ question: q, answer: result }); } catch (e) { }
        return;
      }
    }

    const th = addB(`Searching ${NX.nodes.length} nodes + memory...`, 'ai thinking');
    ...
    try {
      ...
      th.textContent = ans || 'Something went sideways.';
      th.classList.remove('chat-thinking');
      chatHistory.push({ role: 'assistant', content: ans });
      if (voiceOn) speak(ans);
      try { const { error } = await NX.sb.from('chat_history').insert({ question: q, answer: ans }); } catch (e) { }
    } catch (e) {
      th.textContent = e.message || 'Connection hiccup.';
      th.classList.remove('chat-thinking');
    }
  }
  ...
  // ═══ VOICE — 10 ElevenLabs voices ═══
  let pv=null;
  const VOICES=[
    {id:'pNInz6obpgDQGcFmaJgB',name:'Adam',desc:'Sharp & confident'},
    {id:'EXAVITQu4vr4xnSDxMaL',name:'Bella',desc:'Warm & witty'},
    {id:'onwK4e9ZLuTAKqWW03F9',name:'Daniel',desc:'British dry wit'},
    {id:'XB0fDUnXU5powFXDhCwa',name:'Charlotte',desc:'Smart & smooth'},
    {id:'TX3LPaxmHKxFdv7VOQHJ',name:'Liam',desc:'Casual & quick'},
    {id:'jBpfuIE2acCO8z3wKNLl',name:'Emily',desc:'Friendly & clear'},
    {id:'yoZ06aMxZJJ28mfd3POQ',name:'Sam',desc:'Deep & calm'},
    {id:'ThT5KcBeYPX3keUQqHPh',name:'Dorothy',desc:'Warm storyteller'},
    {id:'VR6AewLTigWG4xSOukaG',name:'Arnold',desc:'Bold & direct'},
    {id:'pqHfZKP75CvOlQylNhV4',name:'Bill',desc:'Natural & relaxed'}
  ];
  let cvi=parseInt(localStorage.getItem('nexus_voice_idx')||'0')%VOICES.length;
  function setupVoice(){document.getElementById('micBtn').addEventListener('click',toggleMic);const vb=document.getElementById('voiceBtn');let pt=null;
    ...
    vb.addEventListener('pointerdown',()=>{pt=setTimeout(()=>{cvi=(cvi+1)%VOICES.length;localStorage.setItem('nexus_voice_idx',cvi);voiceOn=true;vb.classList.add('on');vb.title=`Voice: ${VOICES[cvi].name}`;speak(`Hey — ${VOICES[cvi].name} here. ${VOICES[cvi].desc}.`);pt=null;},600);});
    vb.addEventListener('pointerup',()=>{if(pt)clearTimeout(pt);});vb.addEventListener('pointerleave',()=>{if(pt)clearTimeout(pt);});
    if('speechSynthesis'in window){const pk=()=>{const v=speechSynthesis.getVoices();for(const n of['Samantha','Karen','Daniel','Microsoft Aria']){const f=v.find(x=>x.name.includes(n));if(f){pv=f;break;}}};pk();speechSynthesis.onvoiceschanged=pk;}}
