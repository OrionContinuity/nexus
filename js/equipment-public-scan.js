...
(function() {
  ...
  async function submitPmLog(modal, eq, qrCode) {
    ...
    try {
      ...
      // Detect common deployment misconfigs and give a useful error
      // instead of the raw Postgres message. The #1 cause of silent
      // failure in this flow is the pm_logs table not existing — show
      // a clearer message so admins know to run the migration.
      let msg = err?.message || 'Unknown error';
      if (/relation.*pm_logs.*does not exist/i.test(msg) ||
          /could not find.*pm_logs/i.test(msg)) {
        msg = 'The pm_logs table does not exist in the database yet. ' +
              'An admin needs to run the pm_logs_migration.sql file in ' +
              'Supabase. Until that is done, service logs cannot be saved.';
      } else if (/permission denied/i.test(msg) || /row.level security/i.test(msg)) {
        msg = 'Permission denied saving the service log. The pm_logs table ' +
              'needs an RLS policy allowing anonymous inserts. An admin ' +
              'should re-run pm_logs_migration.sql.';
      } else if (/violates check constraint/i.test(msg)) {
        msg = 'Submission rejected by the server. Try refreshing and ' +
              'submitting again. If this keeps happening, contact admin.';
      }
