/* ═══════════════════════════════════════════
   NEXUS — Cleaning Module (cleaning.js)
   With Submit Daily Report integration.
   ═══════════════════════════════════════════ */
(function(){
...
function render(){
  ...
  secs.forEach((sec,si)=>{
    const el=document.createElement('div');el.className='clean-sec'+(si>3?' collapsed':'');
    ...
    h.innerHTML=`<span class="clean-sec-arrow">▼</span><span class="clean-sec-title">${sec.sec}</span><span class="clean-sec-count">${sd}/${sec.items.length}</span>`;
...
// ─── Submit Daily Report → writes to daily_logs ───
async function submitDailyReport(){
  ...
  secs.forEach(sec => {
    ...
    sec.items.forEach((_, i) => {
      total++;
      if (state[loc + '_' + sec.sec + '_' + i]) { done++; sd++; }
    });
    breakdown.push(`${sec.sec}: ${sd}/${sec.items.length}`);
  });

  const pct = total ? Math.round(done / total * 100) : 0;
  const locName = loc.charAt(0).toUpperCase() + loc.slice(1);
  const entry = `Cleaning Report [${locName}]: ${pct}% Complete (${done}/${total}). ${breakdown.join(', ')}.`;
  ...
  try {
    ...
    setTimeout(() => { btn.disabled = false; btn.textContent = '📋 Submit Daily Report'; }, 2500);
  } catch (e) {
