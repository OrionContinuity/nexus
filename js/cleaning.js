/* ═══════════════════════════════════════════
   NEXUS — Cleaning v5
   Big checkmarks, section completion ✅, sticky submit
   ═══════════════════════════════════════════ */
(function(){
...
async function init(){
  ...
  document.querySelectorAll('.clean-tab').forEach(t=>{
    t.addEventListener('click',async()=>{
      ...
      try{const{data}=await NX.sb.from('cleaning_logs').select('*').eq('log_date',today).eq('location',loc);if(data)data.forEach(c=>{state[loc+'_'+c.section+'_'+c.task_index]=c.done;});}catch(e){}
      render();
    });
  });
...
function render(){
  ...
  const secs=DATA[loc]||[];let total=0,done=0;
  secs.forEach((sec,si)=>{
    const el=document.createElement('div');
    let sd=0;sec.items.forEach((_,i)=>{if(state[loc+'_'+sec.sec+'_'+i])sd++;});
    const isComplete=sd===sec.items.length&&sec.items.length>0;
    el.className='clean-sec'+(si>3?' collapsed':'')+(isComplete?' complete':'');

    const h=document.createElement('div');h.className='clean-sec-head';
    h.innerHTML=`<span class="clean-sec-check">${isComplete?'✅':'⬜'}</span><span class="clean-sec-arrow">▼</span><span class="clean-sec-title">${sec.sec}</span><span class="clean-sec-count">${sd}/${sec.items.length}</span>`;
    h.onclick=()=>el.classList.toggle('collapsed');el.appendChild(h);

    const body=document.createElement('div');body.className='clean-sec-body';
    sec.items.forEach((item,i)=>{total++;const k=loc+'_'+sec.sec+'_'+i;const d=!!state[k];if(d)done++;
      ...
      it.onclick=()=>{state[k]=!state[k];render();
...
async function submitDailyReport(){
  const btn=document.getElementById('cleanSubmit');
  const secs=DATA[loc]||[];let total=0,done=0;const bd=[];
  secs.forEach(sec=>{let sd=0;sec.items.forEach((_,i)=>{total++;if(state[loc+'_'+sec.sec+'_'+i]){done++;sd++;}});bd.push(`${sec.sec}: ${sd}/${sec.items.length}`);});
  const pct=total?Math.round(done/total*100):0;
  const locName=loc.charAt(0).toUpperCase()+loc.slice(1);
  const entry=`Cleaning Report [${locName}]: ${pct}% Complete (${done}/${total}). ${bd.join(', ')}.`;
  btn.disabled=true;btn.textContent='Submitting...';
  try{await NX.sb.from('daily_logs').insert({entry});btn.textContent='✓ Submitted';setTimeout(()=>{btn.disabled=false;btn.textContent='📋 Submit Daily Report';},2500);}catch(e){btn.textContent='Error';setTimeout(()=>{btn.disabled=false;btn.textContent='📋 Submit Daily Report';},2500);}
}
