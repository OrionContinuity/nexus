...
(function () {
  ...
  async function open() {
    ...
    list.addEventListener('click', async (e) => {
      const btn = e.target.closest('.wo-btn');
      if (!btn) return;
      const row = btn.closest('.wo-row');
      const issueId = row?.dataset.issue;
      const eqId = row?.dataset.eq;
      if (btn.dataset.act === 'board' && issueId) {
        // Same deep link Home uses — board.js resolves it across boards
        // and self-heals a missing card.
